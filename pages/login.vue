<template>
	<div class="flex items-center justify-center p-4 sm:p-11">
		<div class="max-w-xs sm:max-w-md lg:max-w-3xl">
			<h1
				class="text-yn-deep-purple font-extrabold text-3xl sm:text-4xl lg:text-5xl mb-4 sm:mb-6 text-center"
			>
				Login
			</h1>
			...
			>
				<form @submit.prevent class="space-y-4">
					<div>
						<label for="email" class="text-yn-deep-purple text-sm sm:text-lg"
							>Email</label
						>
						<input
							v-model="creds.email"
					...
					<div>
						<label for="password" class="text-yn-deep-purple text-sm sm:text-lg"
							>Password</label
						>
						<input
						...
						/>
						<div
							v-if="errorMessage.includes('password') && errorMessage.length"
						>
							<p class="text-red-500 text-sm sm:text-lg">{{ errorMessage }}</p>
						</div>
					</div>
	...
	async function loginUser() {
		try {
			...
			if (error instanceof Error) {
				...
				errorMessage.value = 'wrong password or email, try again'
			}
		}
	}
</script>
